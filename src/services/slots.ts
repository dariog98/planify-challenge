...
const getAvalaibleSlots = (serviceId : number | undefined) : Slots[] => {
    ...
    const userReservationStorage : string | null = window.localStorage.getItem('RESERVATIONS')
    const userReservation : Reservation[] = userReservationStorage ? JSON.parse(userReservationStorage) : []
    const unavailableSlots = userReservation.reduce((accumulator: Record<string, string[]>, reservation) => {
